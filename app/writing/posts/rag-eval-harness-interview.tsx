...
export default function Post() {
  return (
    <>
      ...
      <p className="callout">
        Without an eval set, every change to a RAG pipeline is a vibe. With one, every change is
        an experiment. That distinction is the whole interview.
      </p>

      <h2>Build the harness first</h2>

      <p>
        The thing that makes this project defensible is small and takes about a day.
      </p>

      <ol>
        <li>
          <strong>A labelled question set.</strong> Fifty to a hundred and fifty questions over
          your corpus, each tagged with the document (ideally the chunk) that contains the
          answer. Write them by hand. Generating them with a model is fine as a first pass, but
          the model will write questions that are trivially retrievable and your scores will
          be flattering and useless.
        </li>
        <li>
          <strong>Retrieval metrics, measured separately from generation.</strong>{" "}
          Recall@k and MRR over that set. This is the step almost everyone omits, and it is the
          one that matters: if the right chunk is not in the context, no amount of prompt work
          fixes the answer.
        </li>
        <li>
...
PASS  no regression beyond -0.02 threshold`}</pre>
      ...
      <p>
        Re-embedding everything is fine at ten thousand documents and untenable at ten million.
        Incremental indexing, a document-version key, and stale-chunk deletion are the real
        answer. This is the question that separates people who built a demo from people who
        thought about operating it.
      </p>
      ...
      <h2>Scoping it to six weeks</h2>

      <p>
        Pick a corpus you actually care about and that has awkward structure: tables, footnotes,
        near-duplicate revisions. A clean corpus makes the project easy and the interview
        boring.
      </p>
